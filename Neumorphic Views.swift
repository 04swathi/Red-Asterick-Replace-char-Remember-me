...
class NeumorphicViews: UIView {
    ...
    @IBInspectable
    var shadowColor: UIColor = UIColor(red: 168/255, green: 168/255, blue: 168/255, alpha: 1.0) {
        didSet {
            setNeedsDisplay()
        }
    }
    ...
    @IBInspectable
    var shadowOffset: CGSize = CGSize(width: 7, height: 7) {
        didSet {
            setNeedsDisplay()
        }
    }
    ...
    @IBInspectable
    var shadowBlurRadius: CGFloat = 16.0 {
        didSet {
            setNeedsDisplay()
        }
    }
    ...
    override func draw(_ rect: CGRect) {
        ...
        let squarePath = UIBezierPath(roundedRect: rect, cornerRadius: cornerRadius)
        
        if let backgroundColorOverride = backgroundColorOverride {
            backgroundColorOverride.setFill()
        } else {
            UIColor(red: 224/255, green: 224/255, blue: 224/255, alpha: 1.0).setFill()
        }
        
        squarePath.fill()
        
        // Draw the inset box shadow
        let context = UIGraphicsGetCurrentContext()
        context?.setShadow(offset: shadowOffset, blur: shadowBlurRadius, color: shadowColor.cgColor)
        context?.setFillColor(UIColor.white.cgColor)
        
        let shadowRect = rect.insetBy(dx: shadowBlurRadius, dy: shadowBlurRadius)
        context?.fillRoundedRect(rect: shadowRect, cornerRadius: cornerRadius)
    }
